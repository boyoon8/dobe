...
class Otheradmin extends CB_Controller
{
    ...
    public function other_sub_1()
    {
        ...
        $result = $this->{$this->modelname}
            ->get_admin_list('', '', array('ota_type'=> 'other_sub_1'), '', $findex, $forder);
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                $countwhere = array(
                    'ota_id' => element('ota_id', $val),
                );
                // $result['list'][$key]['member_count'] = $this->Other_admin_model->count_by($countwhere);
            }
        }
    ...
    public function other_sub_2()
    {
        ...
        $result = $this->{$this->modelname}
            ->get_admin_list('', '', array('ota_type'=> 'other_sub_2'), '', $findex, $forder);
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                // $result['list'][element('ota_value',$val)][$key]['member_count'] = $this->Other_admin_model->count_by($countwhere);
            }
        }
        $view['view']['data'] = $result;
    ...
    public function other_sub_3()
    {
        ...
        $this->{$this->modelname}->allow_order_field = array('ota_order'); // 정렬이 가능한 필드
        $result = $this->{$this->modelname}
            ->get_admin_list('', '', array('ota_type'=> 'other_sub_3'), '', $findex, $forder);
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                $countwhere = array(
                    'ota_id' => element('ota_id', $val),
