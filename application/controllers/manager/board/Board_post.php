...
class Board_post extends CB_Controller
{
    ...
    public function lists($brd_key = '',$post_id = '')
    {
        ...
        $page_name = str_replace($searchconfig, $replaceconfig, $page_name);
            ...
            $list_skin_file = element('use_gallery_list', element('board', $list)) && empty($this->input->get('latest')) ? 'gallerylist' : 'list';

        // if($brd_key==='notice') $list_skin_file = $list_skin_file.'_'.$brd_key;
        // 
        $list_skin_file = $list_skin_file.'_'.$brd_key;
    ...
    public function post($post_id = 0, $print = false)
    {
        ...
        $view['view']['write_url'] = '';
        if ($can_write === true) {
            if(element('brd_key', $board)==='pds')
                $view['view']['write_url'] = manager_write_url(element('brd_key', $board));
            else
                $view['view']['write_url'] = 'javascript:write_post(\'detail_view\',\''.element('brd_key', $board).'\',\'\',\'\');'; 
        ...
        $view['view']['reply_url'] = ($can_reply === true && ! element('post_del', $post))
            ? manager_reply_url(element('post_id', $post)) : '';
        ...
        if ($skeyword) {
            $view['view']['list_url'] = manager_board_url(element('brd_key', $board));
            $view['view']['search_list_url'] = manager_board_url(element('brd_key', $board) . '?' . $param->output());
        } else {
            $view['view']['list_url'] = manager_board_url(element('brd_key', $board) . '?' . $param->output());
    ...
    public function _get_list($brd_key, $from_view = '')
    {
        ...
        if($start_date) $where = array('post_datetime >=' => $start_date);
        if($end_date) $where = array('post_datetime <=' => $end_date);
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['url'] = 'javascript:view_post(\'detail_view\',\''.element('post_id', $val).'\',\'\',\'\');';
         ...
         * 페이지네이션을 생성합니다
         */
        $config['base_url'] = manager_board_url($brd_key) . '?' . $param->replace('page');
        ...
        if ($skeyword) {
            $return['list_url'] = manager_board_url(element('brd_key', $board));
            $return['search_list_url'] = manager_board_url(element('brd_key', $board) . '?' . $param->output());
        } else {
            $return['list_url'] = manager_board_url(element('brd_key', $board) . '?' . $param->output());
        ...
        if ($can_write === true) {
            if($brd_key==='pds')
                $return['write_url'] = manager_write_url($brd_key);
            else
                $return['write_url'] = 'javascript:write_post(\'detail_view\',\''.$brd_key.'\',\'\',\'\');';
    ...
    public function group_list($bgr_key = '')
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_group_index';
        $this->load->event($eventname);
