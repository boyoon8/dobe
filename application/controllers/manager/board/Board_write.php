...
class Board_write extends CB_Controller
{
    ...
    public function _write_common($board, $origin = '', $reply = '')
    {
        ...
        $view['view']['post']['can_post_hotline'] = $can_post_notice = ($is_admin !== false) ? true : false;
        ...
        if ($form_validation === false OR $file_error) {
                ...
                foreach ($form as $key => $value) {
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        $extra_content[$k]['input'] .= '</div>';
                            ...
                            if(element('field_name', $value) ==='department'){
                                $this->load->model('Member_group_model');
                                $options = $this->Member_group_model->get_all_group();
                                if ($options) {
                                    foreach ($options as $okey => $oval) {
                                        $extra_content[$k]['input'] .= '<option value="' . element('mgr_title', $oval) . '" ' . set_select(element('field_name', $value), element('mgr_title', $oval)) . ' >' . element('mgr_title', $oval) . '</option> ';
                                    }
                                }
                            } else {
                                $options = explode("\n", element('options', $value));
            ...
            if ($reply && $origin && $this->cbconfig->item('use_notification') && $this->cbconfig->item('notification_reply')) {
                ...
                $not_url = manager_post_url(element('brd_key', $board), $post_id);
            ...
            $replaceconfig = array(
                ...
                $updatedata['post_userid'],
                cdate('Y-m-d H:i:s'),
                manager_post_url(element('brd_key', $board), $post_id),
                element('brd_name', $board),
                manager_board_url(element('brd_key', $board)),
            );
            ...
            $redirecturl = manager_board_url(element('brd_key', $board),$post_id);
    ...
    public function modify($post_id = 0)
    {
        ...
        if ($form_validation === false OR $file_error) {
                ...
                foreach ($form as $key => $value) {
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        if(element('field_name', $value) ==='department'){
                            ...
                            if ($options) {
                                foreach ($options as $okey => $oval) {
                                    $extra_content[$k]['input'] .= '<option value="' . element('mgr_title', $oval) . '" ' . set_select(element('field_name', $value), element('mgr_title', $oval),($item === element('mgr_title', $oval) ? true : false)) . ' >' . element('mgr_title', $oval) . '</option> ';
                                }
                            }
                        } else {
                            $options = explode("\n", element('options', $value));
            ...
            $redirecturl = manager_board_url(element('brd_key', $board), $post_id) . '?' . $param->output();
