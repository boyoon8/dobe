...
class Board extends CI_Controller
{
    ...
    public function latest($config)
    {
        ...
        if ($latest && is_array($latest)) {
            foreach ($latest as $key => $value) {
                ...
                $view['view']['latest'][$key]['post_url'] = post_url($brd_key, element('post_id', $value));
                $view['view']['latest'][$key]['url'] = 'javascript:view_post(\'detail_view\',\''.element('post_id', $value).'\',\'\',\'\',\'\',\''.html_escape(element('board_name', element('board', $view['view']))).'\');';
     ...
     * 최근게시물을 가져옵니다
     */
    public function manager_latest($config)
    {
        $view = array();
        $view['view'] = array();
        ...
        $view['view']['board']['gallery_percent'] = floor( 102 / $view['view']['board']['gallery_cols']) - 2;
        ...
        $where['post_del'] = 0;
        $where['post_secret'] = 0;
        if(!empty($post_notice)) $where['post_notice'] = $post_notice;
        $this->CI->db->from('post');
        $this->CI->db->where($where);
        ...
        $view['view']['skinurl'] = base_url( VIEW_DIR .'/'.config_item('uri_segment_manager').'/basic/latest/' . $skin);
        
        $html = $this->CI->load->view(config_item('uri_segment_manager').'/basic/latest/' . $skin . '/latest', $view, true);
